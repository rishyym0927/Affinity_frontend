...
const FormStep3 = ({ onBack }) => {
  ...
  const renderField = (label, value) => (
    <div className="mb-2">
      <span className="text-gray-800 dark:text-gray-300 font-semibold">{label}:</span>
      <span className="ml-2 text-black dark:text-white">{value || 'Not provided'}</span>
    </div>
  );
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-4xl text-black dark:text-white font-bold text-[#ff0059] mb-4">Review and Submit</h2>
        <p className="text-gray-600 dark:text-neutral-400 font-bold mb-4">Take a final look before you <span className="text-yellow-600">join us!</span></p>
      </div>

      <div className="bg-gray-100 dark:bg-neutral-700 p-4 rounded-lg shadow-inner text-xs">
        <div className="grid grid-cols-2 gap-4">
          ...
          {registerInfo.image_url && (
            <div className="mt-4 flex">
              <span className="text-gray-6800 dark:text-gray-300 font-semibold">Profile Picture:</span>
        ...
        <form onSubmit={registerUser} className="space-y-4 mt-4">
          <div className="flex gap-2">
            <button
              type="button"
              onClick={onBack}
              className="bg-gray-300 dark:bg-neutral-900 hover:bg-gray-400 dark:hover:bg-gray-600 text-black dark:text-white py-2 px-4 rounded-md"
            >
              Back
            </button>
            <button
