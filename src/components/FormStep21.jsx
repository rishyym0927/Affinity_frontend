...
const FormStep21 = ({ onNext, onBack }) => {
  ...
  async function onFileSelect(e) {
    try {
      ...
      const response = await axios.get(`${RUST_MAIN_URL}/aws/presigned_url`, {
        params: {
          filename,
        },
      });
      ...
      const image_name = pathParts[pathParts.length - 1];
      ...
      const imageUrl = `https://affinitys3.s3.ap-south-1.amazonaws.com/uploads/${image_name}`;
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-4xl text-white font-bold text-[#ff0059] mb-4">
          More About You
        </h2>
      ...
      <form className="space">
        <label className="block">
          ...
          />
          {errors.interests && (
            <p className="text-[#ff0059]">{errors.interests}</p>
          )}
        </label>
        ...
        <div className="block">
          ...
          {errors.past_relationships && (
            <p className="text-[#ff0059]">{errors.past_relationships}</p>
          )}
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 relative h-28 flex flex-col items-center justify-center bg-neutral-800 border border-gray-600 rounded-md overflow-hidden">
            ...
            {registerInfo.image_url ? (
              <img
                src={registerInfo.image_url}
                alt="Profile"
                className="h-full w-full object-cover"
              />
            ) : (
              <div className="text-white text-xl font-bold">Upload Image</div>
            )}
          </div>
        </div>
      </form>
